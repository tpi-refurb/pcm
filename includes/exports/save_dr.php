<?php
	...
	$tb = decode_url((isset($_GET['tb']) && $_GET['tb'] != '') ? $_GET['tb'] : '0');
	...
	class PDF extends FPDF{
		...
		function print_dr($data,$count,$brand,$status){
			$unit = ($count >1)? 'PCS': 'PC';
			$this->Ln(16);
			$this->Cell(35);
			$this->SetFont('Arial','B',9);
			$this->Cell(50,6,$brand.' ('.$status.')',$this->borders,0,'L',false);			
			$this->Ln(6);
	...
	if(count($rs)){
		$pdf->print_dr($rs,count($rs),$brand,$status);
	}
