...
class Auth
{
	...
	*/
	...
	function isLogin(){		
		...
		*/
		...
		if(isset($_COOKIE[$this->config->cookie_name]) and $this->checkSession($_COOKIE[$this->config->cookie_name])){
			return true;
		}
		return false;
	...
	function getUserid(){
		if(isset($_COOKIE[$this->config->cookie_userid])){
			return decode_url($_COOKIE[$this->config->cookie_userid]);
		}
		return '';		
	}
	
	function getUsername(){
		if(isset($_COOKIE[$this->config->cookie_username])){
			return decode_url($_COOKIE[$this->config->cookie_username]);
		}
		return '';		
	}
	...
	*/
	...
	public function getLevelName($username){
		$level_id = $this->getLevel($username);
		$query = $this->dbh->prepare("SELECT group_name FROM users_group WHERE id = ?");
		$query->execute(array($level_id));

		if($query->rowCount() == 0) {
			return false;
		}
		return $query->fetch(PDO::FETCH_ASSOC)['group_name'];
	}
	...
	*/
	...
	private function addNewUser($email, $password, $repeatpassword,$username,$firstname, $lastname)
	{
		...
		$active =$this->config->pcm_auto_activate; // if auto activate
		
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ?, username=?, password = ?, firstname=?, lastname=?, avatar_color=?, active=?, created_date=? WHERE id = ?");
	
		if(!$query->execute(array($email,$username, $password,$firstname, $lastname,$hex_color,$active,$created_date, $uid))) {
			$query = $this->dbh->prepare("DELETE FROM {$this->config->table_users} WHERE id = ?");
			$query->execute(array($uid));
