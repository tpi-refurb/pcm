<?php	
	require('../setup.php');
	$dr_no		=strtoupper(remove_unwanted_char(isset($_POST['unit_dr_no']) ? $_POST['unit_dr_no']: ''));
	...
	if(count($error) <=0){
		...
		$error['error']= false;
		$error['url']= 'includes/exports/tsr.php?p='.encode_url('20').'&s='.encode_url('s').'&dt='.encode_url($dt).'&st='.encode_url($st).'&bt='.encode_url($bt).'&rt='.encode_url($rt).'&sp='.encode_url($sp).'&tbl='.encode_url($view).'&tc='.encode_url($tech).'&dr='.encode_url($dr_no).'&tp='.encode_url($test_power).'&tf='.encode_url($test_fan).'&tb='.encode_url($test_burn);		
	}else{
		$error['error']= true;
	}
